...
if [[ ! $(ifconfig $INTERFACE) == *"RUNNING"* ]]; then
   echo -e "\033[31mPastikan kabel lan terhubung ke ps4\033[0m" | sudo tee /dev/tty1
   while [[ ! $(ifconfig $INTERFACE) == *"RUNNING"* ]]
   do
      coproc read -t 2 && wait "$!" || true
   done
   echo -e "\033[32mPastikan kabel lan terhubung ke ps4\033[0m\n" | sudo tee /dev/tty1
fi
...
if [[ $GHT == *"open"* ]] ; then
echo -e "\n\033[95mGoldhen tidak ditemukan untuk memulai pppwn\033[0m\n" | sudo tee /dev/tty1
if [[ $LEDACT == "status" ]] ;then
	echo none | sudo tee $PLED >/dev/null
...
echo -e "\n\033[95mSiap untuk koneksi ke konsol\033[0m\n" | sudo tee /dev/tty1
while [ true ]
do
if [[ $LEDACT == "status" ]] ;then
